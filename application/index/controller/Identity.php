...
class Identity extends BaseController
{
    public function index() {
        $res = db('identity')->order('id desc')->select();
    ...
    public function add() {
       ...
       $res = db('identity')->insert(['identity_name' => $name]);
       if($res){
    ...
    public function edit() {
        ...
        if(empty($name)){
         ...
         $res = db('identity')->where('id', '=', $id)->update(['identity_name' => $name]);
         if($res !== false){
    ...
    public function del() {
        ...
        db('identity')->where('id', '=', $id)->delete(); 
